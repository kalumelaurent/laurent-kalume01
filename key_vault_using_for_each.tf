...
# Liste des 5 clusters à créer
variable "ml_clusters" {
  type    = list(string)
  default = ["cpu1", "cpu2", "cpu3", "cpu4", "cpu5"]
}
...
#########################################
#   MACHINE LEARNING COMPUTE CLUSTERS (x5)
#########################################

resource "azurerm_machine_learning_compute_cluster" "ml_cpu" {
  for_each = toset(var.ml_clusters)
...
output "compute_clusters" {
  value = [for c in azurerm_machine_learning_compute_cluster.ml_cpu : c.name]
}
